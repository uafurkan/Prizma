    ...
    <div ref={wrapperRef} className="relative w-full max-w-2xl mx-auto">
      <div
        className={`relative flex items-center rounded-2xl border transition-all duration-300 ${
          focused
            ? 'border-prism-b/50 shadow-[0_0_30px_rgba(77,159,255,0.15)]'
            : 'border-border hover:border-muted/50'
        } bg-surface`}
      >
        <svg
          className="absolute left-4 w-5 h-5 text-muted"
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
          strokeWidth={2}
        ...
        <input
          ...
          className="w-full py-4 pl-12 pr-4 bg-transparent border-none text-foreground placeholder-muted outline-none font-sans text-base"
        />
      </div>

      {/* Sonuçlar dropdown */}
      {focused && results.length > 0 && (
        <div className="absolute top-full left-0 right-0 mt-2 bg-surface border border-border rounded-2xl overflow-hidden shadow-2xl z-50 backdrop-blur-xl">
          {results.map((res) => (
            <Link
              key={res.slug}
              ...
              <div className="flex items-center gap-2">
                <span
                  ...
                  style={{
                    ...
                    backgroundColor: `color-mix(in srgb, ${getFormatRenk(res.from)} 25%, transparent)`,
                  }}
                >
                  {res.from}
                </span>
                ...
                <span
                  ...
                  style={{
                    ...
                    backgroundColor: `color-mix(in srgb, ${getFormatRenk(res.to)} 25%, transparent)`,
                  }}
                >
                  {res.to}
                </span>
