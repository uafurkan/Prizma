          ...
          <header className="border-b border-border bg-background/85 backdrop-blur-md sticky top-0 z-50">
            <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
              ...
              <nav className="flex items-center gap-4 md:gap-6 overflow-x-auto no-scrollbar max-w-[65%] whitespace-nowrap">
                ...
                <Link href={getCategoryPath(lang, 'goruntu')} className="text-sm font-semibold text-muted hover:text-foreground transition-colors">{dict.kategoriler.goruntu}</Link>
                <Link href={getCategoryPath(lang, 'video')} className="text-sm font-semibold text-muted hover:text-foreground transition-colors">{dict.kategoriler.video}</Link>
                <Link href={getCategoryPath(lang, 'ses')} className="text-sm font-semibold text-muted hover:text-foreground transition-colors">{dict.kategoriler.ses}</Link>
                <Link href={getCategoryPath(lang, 'belge')} className="text-sm font-semibold text-muted hover:text-foreground transition-colors">{dict.kategoriler.belge}</Link>
                <Link href={getCategoryPath(lang, 'arsiv')} className="text-sm font-semibold text-muted hover:text-foreground transition-colors">{dict.kategoriler.arsiv}</Link>
                <Link href={getCategoryPath(lang, 'altyazi')} className="text-sm font-semibold text-muted hover:text-foreground transition-colors">{dict.kategoriler.altyazi}</Link>
